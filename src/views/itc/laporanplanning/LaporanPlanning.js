...
const exportDataPlanning = async (e) => {
  ...
  try {
    ...
    // Cek apakah respons valid sebelum membuat blob
    if (response.status === 200) {
      const url = window.URL.createObjectURL(new Blob([response.data], { type: response.headers['content-type'] }));
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", "LAPORAN_PLANNING.xlsx"); // Nama file untuk diunduh
      document.body.appendChild(link);
      link.click();
      link.parentNode.removeChild(link); // Hapus elemen link setelah klik
    } else {
