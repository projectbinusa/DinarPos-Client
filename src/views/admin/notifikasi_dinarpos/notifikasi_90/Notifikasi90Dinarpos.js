...
import {
  ...
  NOTIFIKASI_90_DINARPOS,
  NOTIFIKASI_KONFIRMASI_90_DINARPOS,
} from "../../../../utils/BaseUrl";

function Notifikasi90Dinarpos() {
  ...
  const getAll = async () => {
    try {
      const response = await axios.get(`${NOTIFIKASI_90_DINARPOS}`, {
  ...
  // GET KONFIRMASI 90 HARI
  const getAllKonfirmasi = async () => {
    try {
      const response = await axios.get(`${NOTIFIKASI_KONFIRMASI_90_DINARPOS}`, {
        headers: { "auth-tgh": `jwt ${localStorage.getItem("token")}` },
      });
      ...
      <div className="lg:ml-[18rem] ml-0 pt-24 lg:pt-5 w-full px-5 overflow-x-auto">
        <div className="flex flex-col items-start lg:flex-row lg:items-center lg:justify-between">
          <Typography variant="lead" className="uppercase">
            KETERANGAN NOTIFIKASI 90 HARI dinarpos
          </Typography>
          <Breadcrumbs className="bg-transparent">
            ...
            <a href="/notifikasi_90_dinarpos">
              <span>Notifikasi 90 Dinarpos</span>
            </a>
          </Breadcrumbs>
        </div>
        ...
        <main className="bg-white shadow-lg p-5 my-5 rounded">
          <Typography variant="lead" className="uppercase">
            KONFIRMASI NOTIFIKASI 90 HARI DINARPOS
          </Typography>
                ...
                {konfirmasis.length > 0 ? (
                  konfirmasis.map((row, index) => (
                    <tr key={index}>
                      ...
                      <td className="text-sm py-2 px-3">{row.ket90Hari}</td>
...
export default Notifikasi90Dinarpos;
